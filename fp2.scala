...
object fp2 {
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    def f(att : String, x: String) : String = {att + x + term}
      xs match {
        case Nil      => ""
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    def f(att : String, x: String) : String = {delimiter + att + x}
      xs match {
        case Nil      => ""
  ...
  def Aux(a : Int, xs : List[Int]) : Int = {
    xs match {
      case Nil    => throw new RuntimeException
      case x::xs  => if (x < a) Aux(a, xs) else x
    }
  }
}
