...
object fp3 {
  ...
  def delete1 [X] (x:X, ys:List[X]) : List[X] = {
    ys match {
      case Nil   => Nil
      case y::ys => if (y == x) delete1(x, ys) else y::delete1(x, ys)
    }
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    def aux [X] (x:X, xs:List[X], i:Int) :List[(Int,X)] = {
      xs match {
        case Nil   => if (x == Nil) List() else (i, x)::List()
        case y::ys => if (y == x) aux(x, ys, i+1) else (i, x)::aux(y, ys, 1)}
      }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    (n, xs) match {
      ...
      case (-1, xs)   => (List(), xs)
      case (n, x::xs) => {
        var(list1, list2) = splitAt(n-1, xs)
          if (n > 0) (x::list1, list2)
          else (list1, x::list2)
      }
    }
  }
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
    xs match {
      case Nil   => true
      case z::zs => if (member(z, zs)) false else allDistinct(zs)
    }
  }
  }
